...
if "OVERVIEW" in selected_nav:
    ...
    st.markdown(
        '<div class="section-header">RISK ANALYSIS</div>', unsafe_allow_html=True
    )
    ...
    st.markdown(
        '<div class="section-header">FEATURE ANALYTICS</div>', unsafe_allow_html=True
    )
    ...
    for i, (category, features) in enumerate(feature_categories.items()):
        with tabs[i]:
            ...
            if category_features:
                ...
                for feature in category_features:
                    feature_name = str(feature)
                    tag_html += f'<span class="feature-tag">{feature_name.replace("_", " ").title()}</span>'

                st.markdown(
                    f'<div style="margin-top: 1rem;">{tag_html}</div>',
                    unsafe_allow_html=True,
                )
    ...
    try:
        corr_data = filtered_df[
            clean_features[:15] + ["total_marks"]
        ].corr()  # Limit for readability
        ...
        for i, (feature, corr_val) in enumerate(total_marks_corr.head(5).items()):
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin: 0.5rem 0; padding: 0.5rem; background: rgba(255,255,255,0.4); border-radius: 8px;">
                <span style="color: #1e293b; font-weight: 500;">{feature_name.replace("_", " ").title()}</span>
    ...
    st.markdown(
        '<div class="section-header">PREDICTIVE MODELS</div>', unsafe_allow_html=True
    )
    ...
    if len(filtered_df) > 100:
        ...
        st.success(
            f"✅ Model trained successfully with {len(clean_features)} clean features"
        )
    ...
    st.markdown(
        '<div class="section-header">ACTIONABLE INSIGHTS</div>', unsafe_allow_html=True
    )
    ...
    with col1:
        # Strongest predictor
        correlations = [
            (feature, filtered_df[feature].corr(filtered_df["total_marks"]))
    ...
    # Summary Statistics
    st.markdown(
        '<div class="section-header">STATISTICAL SUMMARY</div>', unsafe_allow_html=True
    )

    summary_stats = (
        filtered_df[clean_features[:10]].describe().round(2)
    )  # Limit for display
